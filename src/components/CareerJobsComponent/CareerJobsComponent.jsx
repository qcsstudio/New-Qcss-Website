          ...
          <form onSubmit={handleSubmit}>
            ...
            <div className=" flex-col flex gap-4 my-8">
              <PhoneInput
                id="phone-input"
                country={"in"}
                value={formData.contactCode + formData.contactNumber}
                onChange={(value, country) => {
                  const dialCode = `+${country.dialCode}`;
                  const number = value.replace(country.dialCode, "");
                  setFormData((prev) => ({
                    ...prev,
                    contactCode: dialCode,
                    contact: number
                  }));
              ...
              style={{}}
              inputClass="rounded-lg  h-[50px] !border-0 focus:outline-none rounded-lg !my-2"
              buttonClass="!border-0 focus:outline-none rounded-lg"
              className="rounded-[4px] pl-4 bg-white outline-0 h-[50px] w-[100px] "
              />
              <input
                style={{ boxShadow: "-1px 6px 9px -9px rgba(66, 68, 90, 1)" }}
                className="rounded-[4px] pl-4 bg-white outline-0 w-full h-[50px] "
                name="location"
